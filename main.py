...
list_notes.config(yscrollcommand=scroll_list.set, cursor="hand2",
                  background="grey", highlightbackground="grey",
...
note_text.config(yscrollcommand=scroll_text.set, background="#F4F6F7",
                 highlightbackground="grey")

text_frame.pack(side=tk.TOP)

button_frame = tk.Frame(window)
photo_add = PhotoImage(file="add.png")
photo_edit = PhotoImage(file="write.png")
photo_delete = PhotoImage(file="delete.png")

btn_save = tk.Button(button_frame, text="Add", command=lambda: save_note(),
                     image=photo_add)
btn_edit = tk.Button(button_frame, text="Update",
                     command=lambda: update_note(),
                     state=tk.DISABLED, image=photo_edit)
btn_delete = tk.Button(button_frame, text="Delete",
                       command=lambda: delete_note(), state=tk.DISABLED,
                       image=photo_delete)
...
def save_note():
    ...
    if len(title) < 1:
        tk.messagebox.showerror(title="ERROR!!!",
                                message="You MUST enter the note title")
        return

    note = note_text.get("1.0", tk.END)
    if len(note.rstrip()) < 1:
        tk.messagebox.showerror(title="ERROR!!!",
                                message="You MUST enter the notes")
        return
    ...
    if title_exist is True:
        tk.messagebox.showerror(
          title="ERROR!!!",                              message="Note title already exists. Please choose a new title")
        return
...
def delete_note():
    ...
    result = tk.messagebox.askquestion(
      "Delete", "Are you sure you want to delete?", icon='warning'
      )
